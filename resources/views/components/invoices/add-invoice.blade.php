<!-- Add invoice Modal -->
<div class="fixed left-0 right-0 z-50 items-center justify-center hidden overflow-x-hidden overflow-y-auto top-4 md:inset-0 h-modal sm:h-full" id="add-invoice-modal">
    <div class="relative w-full h-full max-w-2xl px-4 md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
            ...
            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-700">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-700 dark:hover:text-white" data-modal-toggle="add-invoice-modal" id="addInvoiceBtn">
                ...
                <form id="addInvoiceForm">
                    <div class="grid grid-cols-6 gap-6 mb-5">
                        <div class="col-span-6 sm:col-span-3">
                            <label for="invoice_type" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Invoice Type</label>
                            <select name="invoice_type" id="addInvoiceType" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                    ...
                    <div class="items-center pt-6 rounded-b dark:border-gray-700">
                        <button class="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" type="submit">Add invoice</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    async function createInvoice(event) {
        ...
        const formData = {
            'invoice_type': $('#addInvoiceType').val(),
        ...
        try {
            ...
            if (response.status === 201 && response.data.success === true) {
                toastr.success("Payment invoice created successfully!");
                $('#addInvoiceBtn').click()
                const addModal = document.getElementById('add-invoice-modal');
    ...
    $('#addInvoiceForm').on('submit', createInvoice);
</script>
@endpush